...
namespace ISACE
{
    class Tech_report
    {
        ...
        public void FillTechReportInfo(string query)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                DataSet TechRep_set = new DataSet();
                adapter.Fill(TechRep_set);
                Program.f1.guna2DataGridView6.DataSource = TechRep_set.Tables[0];
                //guna2DataGridView4.DataSource = user_set.Tables[0];
                Program.f1.guna2DataGridView6.Columns[0].Visible = false;

            }
        }
        public void ReportIdList(List<Tuple<string, string>> reportList)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                connection.Open();
                string item = "SELECT * FROM [Reports_type]";
                SQLiteCommand command = new SQLiteCommand(item, connection);
                SQLiteDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    reportList.Add(new Tuple<string, string>(reader[0].ToString(), reader[1].ToString()));
                }
                reader.Close();
                connection.Close();
            }
        ...
        public void ReportQuery(string query)
        {
