...
namespace ISACE
{
    class Tests
    {
        ...
        public void FillTestInfo(string query)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                ...
                adapter.Fill(Test_set);
                Program.f1.guna2DataGridView8.DataSource = Test_set.Tables[0];
                //guna2DataGridView4.DataSource = user_set.Tables[0];
                Program.f1.guna2DataGridView8.Columns[0].Visible = false;

            }
        }
        public void TestIdList(List<Tuple<string, string>> testList)
        {
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                connection.Open();
                string test = "SELECT * FROM [Tests_type]";
                SQLiteCommand command = new SQLiteCommand(test, connection);
                SQLiteDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    testList.Add(new Tuple<string, string>(reader[0].ToString(), reader[1].ToString()));
                }
                reader.Close();
                connection.Close();
            }
